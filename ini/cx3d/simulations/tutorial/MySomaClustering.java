...
public class MySomaClustering extends AbstractLocalBiologyModule {
	
	private String substanceID;
	
	public MySomaClustering(String substanceID) {
...
//		ecm.view.smallWindowRectangle = smallWindowRectangle;
		...
		for(int i = 0; i<1; i++){
			...
			c.getSomaElement().addLocalBiologyModule(new MySomaClustering("Yellow"));
			c.setColorForAllPhysicalObjects(Param.X_SOLID_YELLOW);
		}
		for(int i = 0; i<1; i++){
			Cell c = CellFactory.getCellInstance(randomNoise(50, 3));
			c.getSomaElement().addLocalBiologyModule(new MySomaClustering("Violet"));
